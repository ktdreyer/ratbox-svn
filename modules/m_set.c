...
list_quote_commands(struct Client *source_p)
{
	...
	sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :Available QUOTE SET commands:",
	...
	for (i = 0; set_cmd_table[i].handler; i++)
	{
		...
		if(j > 3)
		{
			sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :%s %s %s %s",
...
quote_autoconnall(struct Client *source_p, int newval)
{
	...
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :AUTOCONNALL is currently %i",
...
quote_floodcount(struct Client *source_p, int newval)
{
	...
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :FLOODCOUNT is currently %i",
...
quote_idletime(struct Client *source_p, int newval)
{
	...
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :IDLETIME is currently %i",
...
quote_operstring(struct Client *source_p, const char *arg)
{
	if(EmptyString(arg))
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :OPERSTRING is currently '%s'",
...
quote_adminstring(struct Client *source_p, const char *arg)
{
	if(EmptyString(arg))
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :ADMINSTRING is currently '%s'",
...
quote_spamnum(struct Client *source_p, int newval)
{
	...
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :SPAMNUM is currently %i",
...
quote_spamtime(struct Client *source_p, int newval)
{
	...
	{
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :SPAMTIME is currently %i",
...
quote_splitmode(struct Client *source_p, char *charval)
{
	if(charval)
	{
		 ...
		 */
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :SPLITMODE is currently %s",
			   me.name, source_p->name,
...
quote_splitnum(struct Client *source_p, int newval)
{
	if(newval >= 0)
	{
		...
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :SPLITNUM is currently %i",
...
quote_splitusers(struct Client *source_p, int newval)
{
	if(newval >= 0)
	{
		...
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :SPLITUSERS is currently %i",
...
mo_set(struct Client *client_p, struct Client *source_p, int parc, const char *parv[])
{
	...
	if(parc > 1)
	{
		 ...
		 */
		sendto_one(source_p, POP_QUEUE, ":%s NOTICE %s :Variable not found.", me.name, parv[0]);
		return 0;
	}
