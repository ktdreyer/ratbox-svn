...
struct Client* LocalClientList = 0;
  ...
  if (!from)
    {
      ...
      cptr->next_local = NULL;
      cptr->prev_local = NULL;
      cptr->dns_reply  = NULL;
 ...
 *        0                if (cptr != sptr)
 *
 * cptr - The local client originating the exit or NULL, if this
 ...
 * comment - Reason for this exit
 */
int exit_client(struct Client* cptr, struct Client* sptr,
      ...
      if (IsClient(sptr))
        {
          ...
          if (IsPerson(sptr))        /* a little extra paranoia */
            {
              if (sptr->prev_local)
                sptr->prev_local->next_local = sptr->next_local;
              else
                {
                  if (LocalClientList == sptr)
                    {
                      LocalClientList = sptr->next_local;
                    }
                }

              if (sptr->next_local)
                sptr->next_local->prev_local = sptr->prev_local;

              sptr->prev_local = sptr->next_local = NULL;
            }
        }
      if (IsServer(sptr))
        {
