 ...
 */
static u_char *prefix_tochar(prefix_t * prefix)
{
	if (prefix == NULL)
		return (NULL);
...
#ifdef NOTYET
static char *prefix_toa2x(prefix_t * prefix, char *buf, int buf_len, int with_len)
{
	static char tmp[6];
	if (prefix == NULL) {
		strcpy(buf, "(NULL)");
...
static char *prefix_toa2(prefix_t * prefix, char *buff, int buf_len)
{
	return (prefix_toa2x(prefix, buff, buf_len, 0));
}
static char *prefix_toa(prefix_t * prefix)
{
#ifdef IPV6
...
#endif
static prefix_t *New_Prefix2(int family, void *dest, int bitlen,
		      prefix_t * prefix)
{
...
static prefix_t *New_Prefix(int family, void *dest, int bitlen)
{
...
/* ascii2prefix
 */
static prefix_t *ascii2prefix(int family, const char *string)
{
...
static prefix_t *Ref_Prefix(prefix_t * prefix)
{
	if (prefix == NULL)
		return (NULL);
...
static void Deref_Prefix(prefix_t * prefix)
{
	if (prefix == NULL)
		return;
...
{
	patricia_tree_t *patricia = MyMalloc(sizeof *patricia);
				       ...
				       prefix_t * prefix)
{
	...
	if (comp_with_mask
	    (prefix_tochar(node->prefix), prefix_tochar(prefix), bitlen)) {
#ifdef PATRICIA_DEBUG
...
patricia_node_t *patricia_lookup(patricia_tree_t * patricia,
				 prefix_t * prefix)
{
	...
	new_node = MyMalloc(sizeof *new_node);
...
#ifdef NOTYET
static patricia_node_t *try_search_exact(patricia_tree_t * tree, char *string)
{
	prefix_t *prefix;
	patricia_node_t *node;
	if ((prefix = ascii2prefix(AF_INET, string)) != NULL) {
