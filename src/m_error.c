 ...
 *      parv[*] = parameters
 */
void m_error(struct Client *client_p, struct Client *server_p,
             int parc, char *parv[])
{
  char* para;
  ...
  if (IsPerson(client_p) || IsUnknown(client_p))
    return;
  if (client_p == server_p)
    sendto_realops_flags(FLAGS_ALL,"ERROR :from %s -- %s",
			 get_client_name(client_p, HIDE_IP), para);
  else
    sendto_realops_flags(FLAGS_ALL,"ERROR :from %s via %s -- %s", server_p->name,
			 get_client_name(client_p, HIDE_IP), para);
}

void mr_error(struct Client *client_p, struct Client *server_p,
              int parc, char *parv[])
{
  char* para;
...
void ms_error(struct Client *client_p, struct Client *server_p,
              int parc, char *parv[])
{
  char* para;
