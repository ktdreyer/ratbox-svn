 ...
 */
struct ConfItem *
find_conf_by_address (const char *name,
		      struct irc_inaddr *addr, int type, int fam, const char *username)
{
			...
			for (b = 128; b >= 0; b -= 16)
			{
				for (arec = atable[hash_ipv6 (addr, b)]; arec; arec = arec->next)
					if(arec->type == (type & ~0x1)
					   ...
					   && (type & 0x1
					       || match (arec->username,
							 username)) && arec->precedence > hprecv)
					{
...
find_kline (struct Client *client_p)
{
	...
	aconf = find_conf_by_address (client_p->host,
				      ...
				      client_p->localClient->aftype, client_p->username);
 ...
 */
struct ConfItem *
find_address_conf (const char *host, const char *user, struct irc_inaddr *ip, int aftype)
{
	...
	for (i = 0; i < ATABLE_SIZE; i++)
		for (arec = atable[i]; arec; arec = arec->next)
			if(arec->type == CONF_CLIENT)
			{
				...
				get_printable_conf (aconf, &name, &host,
						    &pass, &user, &port, &classname);

				sendto_one (client_p,
					    form_str (RPL_STATSILINE),
					    me.name, client_p->name,
					    (IsConfRestricted (aconf)) ? 'i' :
					    'I', name, show_iline_prefix (client_p, aconf, user),
		...
		for (arec = atable[i]; arec; arec = arec->next)
		{
			if(arec->type == CONF_KILL)
			{
				...
				sendto_one (client_p,
					    ...
					    me.name, client_p->name, 'K', host, user, pass);
			}
		}
	}
}
